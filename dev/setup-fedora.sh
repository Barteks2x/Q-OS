...
sudo wget ftp://ftp.gnu.org/gnu/gcc/gcc-5.2.0/gcc-5.2.0.tar.gz
sudo wget ftp://ftp.gnu.org/gnu/binutils/binutils-2.25.1.tar.gz
sudo wget http://isl.gforge.inria.fr/isl-0.12.tar.gz

# unarchive the downloaded binutils and gcc files
sudo tar -xvf gcc-5.2.0.tar.gz
sudo tar -xvf binutils-2.25.1.tar.gz
sudo tar -xvf isl-0.12.tar.gz

# download gmp, mpfr, mpc and isl from the file included in the gcc download
sudo gcc-5.2.0/contrib/download_prerequisites

# delete link files created by download_prerequisites
sudo rm isl mpc mpfr gmp isl-0.14
...
sudo ../binutils-2.25.1/configure --target=$TARGET --prefix="$PREFIX" --with-sysroot --disable-nls --disable-werror
...
sudo ../gcc-5.2.0/configure --target=$TARGET --prefix="$PREFIX" --disable-nls --enable-languages=c,c++ --without-headers
